...
export class GameGateaway
    ...
    async handleConnection(client: SocketWithAuth) {
        ...
        if (!this.gameService.checkValidParticipant(roomName.toString(), client.userId)) {
            client.disconnect()
        }
        ...
        const connectedClients = this.io.adapter.rooms?.get(roomName.toString())?.size ?? 0;
    ...
    async participantSuccess(
        ...
        await this.gameService.participantSuccess(client.userId, dto.gameId, dto.index);
        client.emit("another-participant-success", { index: dto.index })
    }
    ...
    async participantSkip(
        ...
        await this.gameService.participantSkip(client.userId, dto.gameId, dto.index);
        client.emit("another-participant-skip", { index: dto.index })
    }
